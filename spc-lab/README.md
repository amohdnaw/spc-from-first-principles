...
- ![dice](docs/l04_1_dice_to_bell.png) One die is uniform chaos; the average of 30 dice is a bell with σ/√30 the width. The yellow annotations are measured, not drawn: 2.449 → 0.447 → 0.447… each panel’s spread lands exactly on σ/√k.
- ![sqrtn](docs/l04_2_sqrt_n.png) The √n curve itself — and the same process seen through three lenses (individuals vs means-of-5 vs means-of-25). This one curve is why control charts watch subgroup means.
...
- ![null](docs/l06_1_null_distribution.png) The null distribution of x̄ with the exact integral Φ(3)−Φ(−3) = 0.9973 and its tails annotated with the false-alarm arithmetic.
- ![plumbing](docs/l06_2_rbar_plumbing.png) How good is σ̂ = R̄/d₂? 4 000 simulated stable processes: unbiased, but with only 25 subgroups the limits themselves carry real fuzz — why Phase I needs enough data.
...
- ![voices](docs/l08_1_two_voices.png) Both voices on one axis, twice: centered (Cp = width ratio) then drifted (Cpk = nearer gap ÷ 3σ), with dimension-line measurements.
- ![cpk2ppm](docs/l08_2_cpk_to_ppm.png) The exact mapping from Cpk to promised defects, both with and without the 1.5σ drift convention — showing 1.00/1.33/1.67 are just chosen points on a continuous curve.
...
- ![arl](docs/l09_1_arl.png) Simulated ARL vs shift size (8 000 runs/point), with the EWMA limits **calibrated to match Shewhart’s ARL₀ ≈ 368** so the comparison is honest: at 1σ drift, detection in ~10 subgroups vs ~44 — about 4× sooner.
- ![race](docs/l09_2_race.png) One drifting process, both charts racing: Shewhart waits for an extreme point; EWMA’s smoothed line crosses its tighter limit while every raw point still sits inside ±3σ.
